...
import { ApplicationService, ApplicationResponse, ApplicationStatus, ScheduleInterviewRequest } from '../../modules/services/application.service';
...
export class PartnerOfferApplicationsComponent implements OnInit {
  ...
  canReject(status: ApplicationStatus): boolean {
    return status === 'SENT' || status === 'SHORTLISTED' || status === 'INTERVIEW';
  }

  shortlist(app: ApplicationResponse): void { this.updateStatus(app, 'SHORTLISTED'); }
  reject(app: ApplicationResponse): void { this.updateStatus(app, 'REJECTED'); }
  accept(app: ApplicationResponse): void { this.updateStatus(app, 'ACCEPTED'); }

  private updateStatus(app: ApplicationResponse, status: ApplicationStatus): void {
  ...
  openInterview(app: ApplicationResponse): void {
    ...
    this.interviewAtLocal = app.interviewAt ? this.toDatetimeLocal(app.interviewAt) : '';
  ...
  submitInterview(): void {
    ...
    if (!dtLocal) { this.interviewError = 'Interview date & time is required.'; return; }
    if (!meet) { this.interviewError = 'Google Meet link is required.'; return; }
    if (!this.isMeetUrl(meet)) { this.interviewError = 'Please enter a valid Google Meet URL.'; return; }
    ...
    this.applicationService.scheduleInterview(this.interviewApp.id, payload).subscribe({
      next: () => {
  ...
  private isMeetUrl(url: string): boolean {
    ...
    return u.startsWith('https://meet.google.com/') || u.startsWith('http://meet.google.com/');
  }

  private toDatetimeLocal(iso: string): string {
    try { return iso.length >= 16 ? iso.substring(0, 16) : iso; } catch { return ''; }
  }
