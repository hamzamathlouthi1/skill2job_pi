...
export class HrDashboardComponent implements OnInit {
  ...
  // ─── KPIs ─────────────────────────────────────────
  get total(): number { return this.applications.length; }
  get pending(): number { return this.applications.filter(a => a.status === 'PENDING').length; }
  get accepted(): number { return this.applications.filter(a => a.status === 'ACCEPTED').length; }
  get rejected(): number { return this.applications.filter(a => a.status === 'REJECTED').length; }
  ...
  get levelData(): { label: string; value: number; color: string }[] {
    ...
    const junior  = this.applications.filter(a => a.motivation?.length < 100).length;
    const mid     = this.applications.filter(a => a.motivation?.length >= 100 && a.motivation?.length < 200).length;
    const senior  = this.applications.filter(a => a.motivation?.length >= 200).length;
    return [
