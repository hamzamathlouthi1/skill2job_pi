  ...
  saveReview(): void {
    ...
    if (existingRaw) {
      try {
        ...
        review = { appId: this.activeAppId, notes: '', tags: [], priority: 'MEDIUM', updatedAt: now, history: [] };
      }
    } else {
      review = { appId: this.activeAppId, notes: '', tags: [], priority: 'MEDIUM', updatedAt: now, history: [] };
    ...
    if ((review.priority || 'MEDIUM') !== this.reviewPriority) {
      review.history.push({ at: now, text: `⭐ Priority set to ${this.reviewPriority}` });
    }
    ...
    if (oldTags !== newTags) {
      review.history.push({ at: now, text: `🏷️ Tags updated (${tags.length})` });
    }
    ...
    localStorage.setItem(this.reviewKey(this.activeAppId), JSON.stringify(review));
    this.closeReview();
  }
