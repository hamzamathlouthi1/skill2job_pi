...
export class TrainerHelpComponent implements OnInit {
  ...
  // ---- App status (for smart recommendations) ----
  userId: number | null = null;
  appStatus: AppStatus = 'UNKNOWN';
  statusLoading = false;

  // ⚠️ If your backend endpoint differs, change ONLY this baseUrl/route.
  // This is Angular-only; no Spring changes required.
  private appApiBase = 'http://localhost:8087/api/trainer-applications';
  // expected: GET /by-user/{userId} returns application with status
  ...
  loadApplicationStatus(): void {
    ...
    this.http.get<any>(this.appByUserRoute(this.userId)).subscribe({
      ...
      error: (err) => {
        // If no application exists, many backends return 404
  ...
  private getUserIdFromLocalStorage(): number | null {
    const raw = localStorage.getItem('trainer_user_id');
