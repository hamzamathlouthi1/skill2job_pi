...
import { TrainerProfileFoService } from '../../../services/trainer-profile-fo.service';
...
export class TrainerMyProfileComponent implements OnInit {
  ...
  constructor(private profileService: TrainerProfileFoService) {}
    ...
    this.profileService.getMine(this.userId).subscribe({
      next: (data) => {
        ...
        if (!this.draft.headline?.trim()) {
          this.draft.headline = (data?.mainSpeciality ? `${data.mainSpeciality} Trainer` : 'Professional Trainer');
        }
      ...
      error: (err) => {
        ...
        if (err?.status === 404) {
          this.errorMsg = "Your trainer profile is not available yet. Wait for HR approval.";
          return;
        }

        this.errorMsg = `Error (${err?.status || '???'}) : ${err?.error?.message || 'Unable to load profile'}`;
      }
    });
  }
  ...
  get expertiseList(): string[] { return this.toList(this.draft.expertise); }
  get toolsList(): string[] { return this.toList(this.draft.tools); }
  get languagesList(): string[] { return this.toList(this.draft.languages); }
