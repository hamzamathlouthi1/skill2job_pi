...
getCertificateLevel(cert: Certificate): 'BRONZE' | 'SILVER' | 'GOLD' {
  ...
  const score = this.getCertificateScore(cert);   // ← cert, not ev
  if (score >= 90) return 'GOLD';
  if (score >= 70) return 'SILVER';
  return 'BRONZE';
}

getCertificateScore(cert: Certificate): number {   // ← Certificate, not Evaluation
  ...
  downloadCertificate(certificate: Certificate): void {
    ...
    doc.setFontSize(28);
    doc.setFont('helvetica', 'bold');
    doc.text('CERTIFICATE OF ACHIEVEMENT', pageWidth / 2, 95, { align: 'center' });

    // ── Exam title ──
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(22);
    doc.text((examTitle || 'Unknown Exam').toUpperCase(), pageWidth / 2, 145, { align: 'center' });
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(200, 200, 200);
    doc.text('has successfully completed the exam', pageWidth / 2, 258, { align: 'center' });
    ...
    doc.setTextColor(...levelColor);
    doc.setFont('helvetica', 'bold');
    doc.text(`Score: ${score.toFixed(1)}%`, pageWidth / 2, barY + 30, { align: 'center' });

    doc.setFontSize(11);
    doc.setTextColor(180, 180, 180);
    doc.setFont('helvetica', 'normal');
    doc.text(`Passing Score Required: ${passScore}%`, pageWidth / 2, barY + 48, { align: 'center' });
    ...
    doc.setTextColor(...levelColor);
    doc.setFont('helvetica', 'bold');
    doc.text(certificate.certificateCode, pageWidth / 2, 376, { align: 'center' });
    ...
    doc.setTextColor(180, 180, 180);
    doc.setFont('helvetica', 'normal');
    doc.text(`${new Date().getFullYear()}`, pageWidth - 150, pageHeight - 75, { align: 'center' });
