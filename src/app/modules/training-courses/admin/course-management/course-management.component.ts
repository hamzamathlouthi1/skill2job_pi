      ...
      this.courses = (data || []).map((c: any) => {
        ...
        try {
          if (c.imageUrl && !/^https?:\/\//i.test(c.imageUrl)) {
            c.imageUrl = c.imageUrl.startsWith('/') ? 'http://localhost:8089' + c.imageUrl : 'http://localhost:8089/' + c.imageUrl;
          }
          if (c.pdfUrl && !/^https?:\/\//i.test(c.pdfUrl)) {
            c.pdfUrl = c.pdfUrl.startsWith('/') ? 'http://localhost:8089' + c.pdfUrl : 'http://localhost:8089/' + c.pdfUrl;
          }
        } catch (e) {
