...
export class TrainerCoursesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<any[]>(`${this.base}/users/trainers`).subscribe({
      next: (trainers) => {
        const currentTrainer = trainers.find(t => t.username === userInfo.username);
  ...
  loadMyCourses(trainerId: number): void {
    this.loading = true;
    this.http.get<any[]>(`${this.base}/training-courses/trainer/${trainerId}`).subscribe({
      next:  data  => { this.courses = data || []; this.loading = false; },
      error: ()    => { this.courses = [];          this.loading = false; this.error = true; }
    });
  }
