...
export class PaymentService {
  ...
  private readonly BASE = 'http://localhost:8089/api/payments';
  ...
  checkAccess(courseId: number): Observable<{ hasAccess: boolean }> {
    return this.http.get<{ hasAccess: boolean }>(
      `${this.BASE}/check-access`, { params: { courseId: courseId.toString() } }
    );
  }
  ...
  approvePayment(paymentId: number, notes: string = ''): Observable<PaymentResponse> {
    return this.http.put<PaymentResponse>(
      `${this.BASE}/${paymentId}/approve`,
      { notes }
    );
  }

  rejectPayment(paymentId: number, reason: string): Observable<PaymentResponse> {
    return this.http.put<PaymentResponse>(
      `${this.BASE}/${paymentId}/reject`,
      { reason }
    );
  }
