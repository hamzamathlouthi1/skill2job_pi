...
export class PaymentComponent implements OnInit {
  ...
  countries = [
    'Tunisia', 'France', 'Algeria', 'Morocco', 'Belgium',
    'Switzerland', 'Canada', 'United States', 'Other'
  ];
  ...
  loadCourseDetails(): void {
    ...
    this.courseService.getAll().subscribe((courses: any[]) => {
      this.course = (courses || []).find((c: any) => c.id === this.courseId) || null;
    });
  }

  loadWalletBalance(): void {
    this.walletService.getBalance().subscribe({
      next:  (data: any) => { this.walletBalance = data.balance || 0; },
  ...
  applyCoupon(): void {
    ...
    this.http.get<{ valid: boolean; discountPercentage: number; code: string }>(
      `http://localhost:8089/api/coupons/validate?code=${encodeURIComponent(code)}`
    ).subscribe({
      next: (res) => {
        ...
        this.finalPrice    = +(this.basePrice * (1 - res.discountPercentage / 100)).toFixed(2);
  ...
  validateForm(): boolean {
    ...
    if (this.selectedMethod === 'WALLET') {
      ...
      if (this.walletBalance < this.course.pointsPrice) {
        this.errorMessage = `Insufficient balance. You need ${this.course.pointsPrice.toFixed(0)} points but have ${this.walletBalance.toFixed(0)} points.`;
        return false;
      }
    }
  ...
  submitPayment(): void {
    ...
    const paymentData: PaymentRequest = {
      ...
      ...(this.couponApplied && this.couponCode.trim() && { 
    ...
    this.paymentService.initiatePayment(paymentData).subscribe({
      ...
      error: (err: any) => {
        ...
        const msg = err?.error?.error || err?.error?.message || 'Payment failed. Please try again.';
