...
export class EnrolledCourseComponent implements OnInit {
  ...
  buildLessonsFromPdfs(): void {
    ...
    this.lessons = pdfs.length > 0
      ...
      : [{
          index: 0, title: 'Course Overview',
          pdfUrl: null, pdfName: null,
          completed: false, opened: false
        }];
    ...
    this.progressService.getProgress(this.courseId).subscribe({
      next: (result: any) => {
        if (result && result.id) {
          ...
          if (this.progress && (this.progress as any).totalLessons !== actualTotal) {
          ...
          this.lessons.forEach((l, i) => { if (i < completed) l.completed = true; });
          this.currentLessonIndex = Math.min(completed, this.lessons.length - 1);
        } else {
          this.startCourse();
        }
      },
      error: () => { this.startCourse(); }
    });
  }
  ...
  isLessonLocked(index: number):    boolean { return index > (this.progress?.completedLessons ?? 0); }
  isLessonCompleted(index: number): boolean { return index < (this.progress?.completedLessons ?? 0); }
  isLessonActive(index: number):    boolean { return index === this.currentLessonIndex; }
  getCurrentLesson(): PdfLesson | null       { return this.lessons[this.currentLessonIndex] || null; }
  ...
  openPdf(lesson: PdfLesson): void {
    ...
    const url = lesson.pdfUrl.startsWith('http')
      ? lesson.pdfUrl
      : 'http://localhost:8089' + lesson.pdfUrl;
    ...
    this.progressService.updateProgress(this.courseId, lessonNumber).subscribe({
      next: (updated: CourseProgress) => {
        ...
        if (updated.isCompleted) {
          ...
          setTimeout(() => { this.currentLessonIndex++; }, 600);
        }
      },
  ...
  goBack(): void { this.router.navigate(['/user/courses']); }

  downloadPdf(pdfUrl: string): void {
    const url = pdfUrl.startsWith('http') ? pdfUrl : 'http://localhost:8089' + pdfUrl;
    window.open(url, '_blank');
  }
}
