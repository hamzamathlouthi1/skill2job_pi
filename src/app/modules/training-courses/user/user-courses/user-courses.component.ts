...
import { CourseProgressService, CourseProgress } from '../../course-progress/services/course-progress.service';
...
export class UserCoursesComponent implements OnInit {
  ...
  loadAllProgress(): void {
    ...
    this.progressService.getMyProgress().subscribe({
      ...
      error: () => { this.courseProgressMap.clear(); }
    });
  }
    ...
    this.paymentService.getMyPayments().subscribe({
      next: (payments: any[]) => {
        ...
        if (this.courses.length > 0) {
          this.myCourses = this.courses.filter(c => approvedIds.includes(Number(c.id)));
          this.myCoursesLoading = false;
        } else {
          this.courseService.getAll().subscribe({
            next: (data: any[]) => {
              this.myCourses = (data || []).filter(c => approvedIds.includes(Number(c.id)));
              this.myCoursesLoading = false;
            },
            error: () => { this.myCourses = []; this.myCoursesLoading = false; }
          });
        }
      },
      error: () => { this.myCourses = []; this.myCoursesLoading = false; }
  ...
  loadMyPayments(): void {
    ...
    this.paymentService.getMyPayments().subscribe({
      next: (data: any[]) => {
        this.myPayments = (data || []).sort((a: any, b: any) =>
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        );
        this.paymentsLoading = false;
      },
      error: () => { this.myPayments = []; this.paymentsLoading = false; }
    });
  }
  ...
  loadPaidCourses(): void {
    ...
    this.paymentService.getMyPayments().subscribe({
      ...
      error: () => { this.paidCourseIds = new Set(); }
    });
  }
  ...
  closeCatDropdown(): void { this.isCatDropdownOpen = false; }

  getSelectedCategoryName(): string {
    if (this.selectedCategoryId === null) return 'Categories';
    return this.categories.find((c: any) => c.id === this.selectedCategoryId)?.name || 'Categories';
  }
  ...
  closePriceDropdown(): void { this.isPriceDropdownOpen = false; }
  ...
  applyFilters(): void {
    ...
    this.filteredCourses = this.courses.filter((course: any) => {
      const matchesCategory = this.selectedCategoryId === null || course.category?.id === this.selectedCategoryId;
  ...
  getDisplayPrice(course: any): number {
    ...
    return this.currencyConversion.convert(rawPrice, fromCurrency, targetCurrency);
  }
  ...
  loadCourses(): void {
    this.courseService.getAll().subscribe({
      next: (data: any[]) => {
        this.courses = data || [];
        const prices = this.courses.map(c => this.parsePrice(c.price)).filter(p => p > 0);
        if (prices.length > 0) {
          ...
          this.absoluteMin = 0; this.absoluteMax = 1000;
          this.priceMin    = 0; this.priceMax    = 1000;
        }
        this.filteredCourses = [...this.courses];
        this.currentPage     = 1;
        this.updatePage();
      },
      error: () => { this.courses = []; this.filteredCourses = []; this.pagedCourses = []; }
    });
  }
  ...
  getImageUrl(course: any): string {
    ...
    return course.imageUrl.startsWith('http')
      ? course.imageUrl
      : 'http://localhost:8089' + course.imageUrl;
  }
