...
export class ExamDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('sessionStorage value:', sessionStorage.getItem(`exam_submission_${examId}`));
  ...
  this.examService.getUserEvaluations(userId).subscribe({
    next: (evaluations: Evaluation[]) => {
      console.log('RAW evaluations from backend:', JSON.stringify(evaluations));
  ...
  this.questions.forEach((q) => {
    ...
    const correct =
      userAnswer !== '—' &&
      this.normalizeAnswer(userAnswer) === this.normalizeAnswer(q.correctAnswer);
