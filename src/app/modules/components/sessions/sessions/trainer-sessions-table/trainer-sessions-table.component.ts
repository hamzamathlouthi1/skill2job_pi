...
export class TrainerSessionsTableComponent implements OnInit {
  ...
  loadTrainerSessions(): void {
    ...
    this.sessionsService.getSessionsByTrainerId(this.currentUserId).subscribe({
      next: (data) => {
        ...
        console.log(`Loaded ${this.sessions.length} sessions for trainer ID ${this.currentUserId}`);
      },
      error: (err) => {
    ...
    this.sessionsService.getAllSessions().subscribe({
      next: (allSessions) => {
        ...
        this.sessions = allSessions.filter(session => {
          ...
          return session.user?.id === this.currentUserId ||
                 (session as any).trainerId === this.currentUserId ||
                 (session as any).userId === this.currentUserId;
        });
        
        this.loadingSessions = false;
        console.log(`Filtered ${this.sessions.length} sessions for trainer ID ${this.currentUserId}`);
      },
      error: (err) => {
  ...
  private combineDateAndTime(date: Date | null, timeStr: string): string | null {
  ...
  private updateStartDateTime() {
    this.newSession.startAt = this.combineDateAndTime(this.startDateObj, this.startTimeStr);
  ...
  private updateEndDateTime() {
    this.newSession.endAt = this.combineDateAndTime(this.endDateObj, this.endTimeStr);
  ...
  editSession(session: Session) {
    ...
    this.startTimeStr = session.startAt ? new Date(session.startAt).toTimeString().slice(0,5) : '09:00';
    this.endDateObj = session.endAt ? new Date(session.endAt) : null;
    this.endTimeStr = session.endAt ? new Date(session.endAt).toTimeString().slice(0,5) : '11:00';
    ...
    if (this.currentStep === 2) {
      ...
      if (this.newSession.startAt && this.newSession.endAt) {
        ...
        if (!this.isSessionDurationValid()) {
          this.errors['duration'] = 'Session must be between 0 and 2 hours maximum';
        }
      }
  ...
  validateForm(): boolean {
    ...
    if (!this.newSession.trainerId) {
      ...
      if (!this.newSession.trainerId) {
        this.notify.error('Trainer information missing. Please ensure you are logged in.');
        return false;
      }
    }
  ...
  saveSession() {
    ...
    const payload: any = {
      ...
      sessionEquipments: this.newSession.type === 'ONSITE'
    ...
    if (this.isEditing && this.editingSessionId != null) {
      this.sessionsService.updateSession(this.editingSessionId, payload).subscribe({
        next: res => {
          this.loading = false;
