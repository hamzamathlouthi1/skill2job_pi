...
export class LiveMeetComponent implements OnInit, OnDestroy {
  ...
  async initMedia() {
    try {
      ...
      if (error instanceof DOMException) {
        if (error.name === 'NotAllowedError') alert('Microphone access denied. Please allow and refresh.');
  ...
  connectWebSocket() {
    ...
    this.stompClient = new Client({
      webSocketFactory: () => new SockJS(`http://localhost:8089/ws?token=${token}`),
  ...
  async handleSignal(message: any) {
    ...
    switch (message.type) {
      ...
      case 'CAMERA_OFF':
        ...
        if (pOff) { pOff.cameraOn = false; pOff.stream = undefined; }
        this.cdr.detectChanges();
        break;
    }
  }
  ...
  createPeerConnection(remoteUsername: string): RTCPeerConnection {
    ...
    pc.ontrack = (event) => {
      console.log('📡 Remote track received from', remoteUsername, event.track.kind);
  ...
  closePeerConnection(remoteUsername: string) {
    ...
    if (pc) { pc.close(); this.peerConnections.delete(remoteUsername); }
  }
  ...
  sendSignal(payload: any) {
    if (this.stompClient?.connected) {
      this.stompClient.publish({
        ...
        body: JSON.stringify({ ...payload, roomCode: this.roomCode, from: this.username })
      });
    }
  }
  ...
  sendPrivateSignal(toUsername: string, payload: any) {
    ...
    if (this.stompClient?.connected) {
      this.stompClient.publish({
        destination: '/app/signal',
        body: JSON.stringify({ ...payload, roomCode: this.roomCode, from: this.username, to: toUsername })
      });
    }
  }
  ...
  this.sessionsService.leaveSession(this.sessionId).subscribe({
    ...
    error: (err) => {
      console.warn('Leave skipped (trainer or not a participant):', err.error);
      this.cleanup();
      if (this.isTrainer) {
