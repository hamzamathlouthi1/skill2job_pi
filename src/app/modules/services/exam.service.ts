...
import { Exam, Question, Evaluation, Certificate, ExamSubmission } from '../models/exam';
...
export class ExamService {
  ...
  getExamWithQuestions(examId: number): Observable<{ exam: Exam; questions: Question[] }> {
  ...
  getUserEvaluations(userId: number): Observable<Evaluation[]> {
    return this.http.get<any[]>(`${this.apiUrl}/users/${userId}/evaluations`).pipe(
      map(evaluations => evaluations.map(e => ({
        ...e,
