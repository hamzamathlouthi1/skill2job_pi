...
// ✅ Interface pour la réponse JWT - UPDATED to include id
export interface JwtResponse {
  token: string;
  type: string;
  id: number;           // Added this field to match your backend
  username: string;
  email: string;
  roles: string[];
}
...
export class AuthService {
  ...
  login(username: string, password: string): Observable<JwtResponse> {
    return this.http.post<JwtResponse>(`${this.baseUrl}/auth/login`, {
      ...
      tap(response => {
        // ✅ Save token and user info using saveToken method
        this.saveToken(response.token, response);
        console.log("User saved with ID:", response.id); // Now ID will be logged
      })
    );
  }
