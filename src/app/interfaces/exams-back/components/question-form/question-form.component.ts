...
export class QuestionFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.examId = this.route.snapshot.params['examId'] ? Number(this.route.snapshot.params['examId']) : null;
    this.questionId = this.route.snapshot.params['id'] ? Number(this.route.snapshot.params['id']) : null;
    this.isEditing = !!this.questionId;
    
    if (!this.examId && !this.isEditing) {
      this.errorMessage = 'Exam ID is required';
      setTimeout(() => {
  ...
  onSubmit(): void {
    ...
    if (this.isEditing) {
      this.examsService.updateQuestion(this.questionId!, questionData).subscribe({
        next: (response) => {
