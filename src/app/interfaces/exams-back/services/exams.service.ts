...
export class ExamsService {
  private apiUrl = 'http://localhost:8089/api/exams';
  ...
  checkAnswer(questionId: number, answer: string): Observable<boolean> {
    return this.http.get<boolean>(`${this.apiUrl}/questions/${questionId}/check?answer=${answer}`);
  }
  ...
  getEvaluationsByExam(examId: number): Observable<Evaluation[]> {
    return this.http.get<Evaluation[]>(`${this.apiUrl}/exams/${examId}/evaluations`);
  }

  getEvaluationsByUser(userId: number): Observable<Evaluation[]> {
    return this.http.get<Evaluation[]>(`${this.apiUrl}/users/${userId}/evaluations`);
  }
  ...
  createEvaluation(examId: number, evaluation: Evaluation): Observable<Evaluation> {
    return this.http.post<Evaluation>(`${this.apiUrl}/exams/${examId}/evaluations`, evaluation);
  }
  ...
  getCertificateByEvaluation(evaluationId: number): Observable<Certificate> {
    return this.http.get<Certificate>(`${this.apiUrl}/evaluations/${evaluationId}/certificate`);
  }

  getCertificateByCode(code: string): Observable<Certificate> {
    return this.http.get<Certificate>(`${this.apiUrl}/certificates/code/${code}`);
  }

  generateCertificate(evaluationId: number): Observable<Certificate> {
    return this.http.post<Certificate>(`${this.apiUrl}/evaluations/${evaluationId}/certificate`, {});
  }
