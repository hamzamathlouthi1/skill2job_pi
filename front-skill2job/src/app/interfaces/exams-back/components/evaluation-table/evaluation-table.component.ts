...
export class EvaluationTableComponent implements OnInit {
  ...
  loadEvaluations(): void {
    ...
    request.subscribe({
      next: (data) => {
        ...
        const ids = [...new Set(data.map((e: any) => e.userId).filter(Boolean))];
  ...
  loadUserNames(userIds: number[]): void {
    ...
    userIds.forEach(id => {
      if (!this.userNames.has(id)) {
        this.http.get<any>(`${this.apiBase}/api/admin/users/${id}`, { headers }).subscribe({
  ...
  generateCertificate(evaluationId: number): void {
    this.examsService.generateCertificate(evaluationId).subscribe({
      ...
      error: () => alert('Failed to generate certificate. It may already exist.')
    });
  }
