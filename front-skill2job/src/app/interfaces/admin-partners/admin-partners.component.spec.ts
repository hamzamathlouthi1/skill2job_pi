...
describe('AdminPartnersComponent', () => {
  ...
  beforeEach(async () => {
    partnerServiceSpy = jasmine.createSpyObj('PartnerService', ['getAllPartners', 'updatePartnerStatus', 'deletePartnerById', 'exportPartnersPdf']);
  ...
  beforeEach(() => {
    ...
    partnerServiceSpy.getAllPartners.and.returnValue(of([{ 
      id: 1, 
  ...
  it('should change status successfully', () => {
    partnerServiceSpy.updatePartnerStatus.and.returnValue(of({
      id: 1,
      companyName: 'Partner 1',
      industry: 'IT',
      companyEmail: 'p1@test.com',
    ...
    expect(partnerServiceSpy.updatePartnerStatus).toHaveBeenCalledWith(1, 'APPROVED');
  ...
  it('should handle error when loading partners', () => {
    ...
    partnerServiceSpy.getAllPartners.and.returnValue(throwError(() => new Error('Error')));
