...
type AppStatus = 'NOT_SUBMITTED' | 'PENDING' | 'ACCEPTED' | 'REJECTED' | 'UNKNOWN';

type FaqCategory = 'Getting Started' | 'Application' | 'My Profile' | 'Courses' | 'Account' | 'Troubleshooting';
...
interface SupportTicket {
  ...
  createdAt: string;
  subject: string;
  category: 'Application' | 'Profile' | 'Courses' | 'Payments' | 'Account' | 'Other';
  message: string;
...
export class TrainerHelpComponent implements OnInit {
  ...
  faq: FaqItem[] = [
    {
      ...
      answer: 'Submit your application in the Apply page. HR will review it and you will receive a decision (Accepted/Rejected). Once accepted, your trainer profile becomes available.',
    ...
    },
    {
      ...
      answer: 'You can edit your CV link and motivation only while your application is Pending. Once HR makes a decision, editing is disabled.',
      tags: ['edit', 'pending', 'motivation', 'cv']
    },
    {
      id: 'app-2',
      category: 'Application',
      question: 'I submitted an application but I cannot see it. What should I do?',
      answer: 'Go to My Application and click Refresh. If it still does not appear, sign out and sign back in to ensure your session is correct.',
    ...
    },
    {
      ...
      answer: 'AI analysis data may take a moment to sync. Use the "Refresh AI" button in My Profile. If you still do not see changes, check that you are logged in with the same account that submitted the application.',
    ...
    },
    {
      ...
      answer: 'Your trainer profile is generated automatically after your application is Accepted. Before acceptance, My Profile may show "Not available yet".',
    ...
    },
    {
      ...
      answer: 'Course creation is available only for approved trainers (Accepted). If your application is still Pending, you must wait for HR approval.',
    ...
    },
    {
      ...
      answer: 'This usually happens when your session token is missing/expired. Sign in again and ensure your user ID is stored correctly in localStorage.',
    ...
    },
    {
      ...
      answer: 'Try Refresh, then clear browser cache, then sign out/sign in. If the issue persists, send a support request below with a screenshot and your steps.',
  ...
  ngOnInit(): void {
    // ✅ Directly use AuthService — single source of truth
  ...
  loadApplicationStatus(): void {
    ...
    this.http.get<any>(this.appByUserRoute(this.userId)).subscribe({
      next: (app) => {
      ...
      error: (err) => {
        if (err?.status === 404) this.appStatus = 'NOT_SUBMITTED';
        else this.appStatus = 'UNKNOWN';
        this.statusLoading = false;
      }
    });
  ...
  get filteredFaq(): FaqItem[] {
    ...
    return this.faq.filter(item => {
      const inCategory = this.selectedCategory === 'All' || item.category === this.selectedCategory;
  ...
  submitTicket(): void {
    ...
    const ticket: SupportTicket = {
      id: `TCK-${Math.random().toString(16).slice(2, 8).toUpperCase()}`,
  ...
  get statusBadge(): { label: string; cls: string } {
    switch (this.appStatus) {
      case 'NOT_SUBMITTED': return { label: 'Not submitted', cls: 'neutral' };
      case 'PENDING': return { label: 'Pending', cls: 'wait' };
      case 'ACCEPTED': return { label: 'Accepted', cls: 'ok' };
      case 'REJECTED': return { label: 'Rejected', cls: 'bad' };
      default: return { label: 'Unknown', cls: 'neutral' };
    }
  }
}