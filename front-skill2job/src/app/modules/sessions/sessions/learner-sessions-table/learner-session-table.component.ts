...
export class LearnerSessionTableComponent implements OnInit {
  ...
  updateFilterCounts(): void {
    this.filters.forEach(filter => {
      if (filter.value === 'all') {
        ...
        filter.count = this.sessions.filter(s => 
          this.getSessionStatus(s).toLowerCase() === filter.value
        ).length;
      }
    });
  }
  ...
  applyFilters(): void {
    ...
    if (this.activeFilter !== 'all') {
      filtered = filtered.filter(s => 
        this.getSessionStatus(s).toLowerCase() === this.activeFilter
      );
    }
  ...
  getTotalParticipants(): number {
    return this.sessions.reduce((total, session) => 
      total + (session.participants?.length || 0), 0
    );
  }
