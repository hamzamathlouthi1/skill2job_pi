...
describe('SalleFormComponent', () => {
  ...
  beforeEach(async () => {
    salleServiceSpy = jasmine.createSpyObj('SalleService', ['getById', 'add', 'update']);
    blocServiceSpy = jasmine.createSpyObj('BlocService', ['getAll']);

    salleServiceSpy.getById.and.returnValue(of({ name: '', capacity: 0, status: 'available' }));
    blocServiceSpy.getAll.and.returnValue(of([]));

    await TestBed.configureTestingModule({
      imports: [SalleFormComponent, HttpClientTestingModule, RouterTestingModule],
      providers: [
