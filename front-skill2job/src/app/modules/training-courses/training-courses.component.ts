...
export class TrainingCoursesComponent implements OnInit {
  ...
  loadTrainers(): void {
    this.http.get<TrainerUser[]>(`${environment.apiUrl}/users/trainers`).subscribe({
  ...
  loadCoupons(): void {
    ...
    this.couponService.getAll().subscribe({
      next:  data  => { this.coupons = data || []; this.couponsLoading = false; },
      error: ()    => { this.coupons = [];          this.couponsLoading = false; }
    });
  }
  ...
  saveCoupon(): void {
    ...
    if (!this.newCoupon.discountPercentage
        || this.newCoupon.discountPercentage <= 0
        || this.newCoupon.discountPercentage > 100)   return alert('Discount must be 1–100 %');
    ...
    req.subscribe({
      next:  ()    => { this.loadCoupons(); this.cancelCouponEdit(); },
  ...
  deleteCoupon(id: number): void {
    ...
    this.couponService.delete(id).subscribe({
      next:  () => { this.couponActionLoading[id] = false; this.loadCoupons(); },
      error: () => { this.couponActionLoading[id] = false; }
    });
  }
  ...
  get activeCouponsCount(): number {
    return this.coupons.filter(c => this.getCouponStatus(c) === 'active').length;
  }
  ...
  loadPayments(): void {
    ...
    this.paymentService.getAllPayments().subscribe({
      next: (data: any[]) => {
        this.payments = (data || []).sort((a: any, b: any) =>
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        );
        this.paymentsLoading = false;
      },
      error: () => { this.payments = []; this.paymentsLoading = false; }
    });
  }
  ...
  approvePayment(paymentId: number): void {
    ...
    this.paymentService.approvePayment(paymentId, notes).subscribe({
      next:  ()    => { this.paymentActionLoading[paymentId] = false; this.loadPayments(); },
      error: (err) => {
  ...
  closeRejectModal(): void { this.showRejectModal = null; }
  ...
  confirmReject(paymentId: number): void {
    ...
    this.paymentService.rejectPayment(paymentId, reason).subscribe({
      next:  () => { this.paymentActionLoading[paymentId] = false; this.showRejectModal = null; this.loadPayments(); },
      error: () => { this.paymentActionLoading[paymentId] = false; }
    });
  }
  ...
  get pendingPaymentsCount(): number { return this.payments.filter(p => p.status === 'PENDING').length;  }
  get approvedCount():        number { return this.payments.filter(p => p.status === 'APPROVED').length; }
  ...
  onCategoryChange(event: Event): void {
    ...
    this.newCourse.category = found ? { id: found.id, name: found.name } : { id: null, name: '' };
  }
  ...
  addCourse(): void {
    ...
    if (!this.newCourse.price || this.newCourse.price <= 0)  return alert('Invalid price');
