...
export class CourseManagementComponent implements OnInit {
      ...
      this.courses = (data || []).map((c: any) => {
        ...
        try {
          if (c.imageUrl && !/^https?:\/\//i.test(c.imageUrl)) {
            c.imageUrl = c.imageUrl.startsWith('/') ? environment.apiUrl.replace('/api', '') + c.imageUrl : environment.apiUrl.replace('/api', '') + '/' + c.imageUrl;
          }
          if (c.pdfUrl && !/^https?:\/\//i.test(c.pdfUrl)) {
            c.pdfUrl = c.pdfUrl.startsWith('/') ? environment.apiUrl.replace('/api', '') + c.pdfUrl : environment.apiUrl.replace('/api', '') + '/' + c.pdfUrl;
          }
        } catch (e) {
  ...
  addCourse() {
    ...
    if (this.selectedPdf) fd.append('pdf', this.selectedPdf, this.selectedPdf.name);
  ...
  clearFiles() {
    ...
    const imgEl = document.getElementById('image-input') as HTMLInputElement | null;
    if (imgEl) imgEl.value = '';
    const pdfEl = document.getElementById('pdf-input') as HTMLInputElement | null;
