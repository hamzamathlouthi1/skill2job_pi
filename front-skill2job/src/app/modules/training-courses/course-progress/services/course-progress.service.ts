...
export class CourseProgressService {
  ...
  startCourse(courseId: number, totalLessons: number): Observable<CourseProgress> {
  ...
  updateProgress(courseId: number, lessonNumber: number): Observable<CourseProgress> {
  ...
  getProgress(courseId: number): Observable<CourseProgress | { exists: boolean }> {
    return this.http.get<CourseProgress | { exists: boolean }>(`${this.apiUrl}/course/${courseId}`);
  }
  ...
  resetProgress(courseId: number): Observable<CourseProgress> {
    return this.http.put<CourseProgress>(`${this.apiUrl}/reset/${courseId}`, {});
  }
}