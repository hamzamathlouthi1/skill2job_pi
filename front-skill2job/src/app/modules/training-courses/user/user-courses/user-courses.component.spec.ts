...
describe('UserCoursesComponent', () => {
  ...
  beforeEach(async () => {
    courseServiceSpy = jasmine.createSpyObj('TrainingCourseService', ['getAll']);
    categoryServiceSpy = jasmine.createSpyObj('CategoryService', ['getAll']);
    authServiceSpy = jasmine.createSpyObj('AuthService', ['isLoggedIn']);
    paymentServiceSpy = jasmine.createSpyObj('PaymentService', ['getMyPayments']);
    progressServiceSpy = jasmine.createSpyObj('CourseProgressService', ['getMyProgress']);
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: CurrencyConversionService, useValue: { convert: (v: number) => v } }
      ],
  ...
  beforeEach(() => {
    ...
    // Default mocks
    courseServiceSpy.getAll.and.returnValue(of([{ id: 1, title: 'Java', price: 100, category: { id: 10, name: 'IT', description: 'IT Tech' } }]));
    categoryServiceSpy.getAll.and.returnValue(of([{ id: 10, name: 'IT', description: 'IT Tech' }]));
  ...
  it('should navigate to payment if user is logged in but has no access', () => {
    ...
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/user/payment'], jasmine.any(Object));
  });
