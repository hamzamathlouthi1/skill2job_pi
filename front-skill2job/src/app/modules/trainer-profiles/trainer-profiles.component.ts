...
export class TrainerProfilesComponent implements OnInit {
  ...
  get totalCount():     number { return this.profiles?.length || 0; }
  get activeCount():    number { return (this.profiles || []).filter(p => p?.status === 'ACTIVE').length; }
  get suspendedCount(): number { return (this.profiles || []).filter(p => p?.status === 'SUSPENDED').length; }

  changeStatus(id: number, status: ProfileStatus) {
    this.service.changeStatus(id, status).subscribe({
      next: () => this.load(),
      error: (err: any) => { console.error(err); alert('Error updating status'); }
    });
  }
  ...
  delete(id: number) {
    ...
    this.service.delete(id).subscribe({
      next: () => this.load(),
      error: (err: any) => { console.error(err); alert('Error deleting profile'); }
    });
  }
    ...
    this.http.get<any[]>(`http://localhost:8090/api/admin/trainer-details`)
      .subscribe({
        next: (data) => {
          ...
          const found = (data || []).find(d => d?.application?.userId === userId);
  ...
  get skillsList(): string[] {
    ...
    return this.selectedDetails.skills.split(',').map((s: string) => s.trim()).filter(Boolean);
  }
